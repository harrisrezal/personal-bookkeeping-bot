...
async function handleDescriptionInput(deps, message) {
  ...
  const match = allPending.find((p) =>
    ...
    !isExpired(p.expires_at, deps.now().getTime())
  );
...
async function handleExpenseInput(deps, message) {
  ...
  if (!parsed || parsed.amount <= 0 || !parsed.description) {
    await deps.telegram.sendMessage(chatId, 'Please enter amount and description, e.g. 55 Coffee or Coffee $5');
    return true;
  }
...
function parseTransactionCommand(text, expectedType) {
  ...
  const parsed = parseAmountAndDescription(rest);
  if (!parsed) {
...
// Finds the amount wherever it sits in free text ("Coffee 5", "5 Coffee",
// "Coffee for $5", "$5 for Coffee" all work) instead of assuming it's
// always the first word — a stray word before the amount fails to parse
// and the whole entry gets rejected otherwise.
function parseAmountAndDescription(text) {
  ...
  // Prefer an explicit "$"-prefixed token — it's an unambiguous signal even
  // when the description also contains a number ("Room 12 $85").
  let amountIndex = tokens.findIndex((token) => token.startsWith('$') && parseMoneyAmount(token) !== null);
  if (amountIndex === -1) {
    amountIndex = tokens.findIndex((token) => parseMoneyAmount(token) !== null);
  }
  if (amountIndex === -1) {
    return null;
  }

  const amount = parseMoneyAmount(tokens[amountIndex]);
  if (!amount || amount <= 0) {
    return null;
  }

  // Drop a connector word left dangling right next to the amount so
  // "Coffee for $5" / "$5 for Coffee" both read as description "Coffee".
  const excluded = new Set([amountIndex]);
  const beforeIndex = amountIndex - 1;
  const afterIndex = amountIndex + 1;
  if (beforeIndex >= 0 && /^for$/i.test(tokens[beforeIndex])) {
    excluded.add(beforeIndex);
  } else if (afterIndex < tokens.length && /^for$/i.test(tokens[afterIndex])) {
    excluded.add(afterIndex);
  }

  const description = tokens.filter((_, index) => !excluded.has(index)).join(' ').trim();
